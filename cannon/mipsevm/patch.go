...
func PatchRust(st *State) error {
	// place the loader at the start of the memory after appending the program entry point
	loader := bytes.NewBuffer([]byte{0x3c, 0x1d, 0x7f, 0xff, 0x37, 0xbd, 0xd0, 0x0, 0x34, 0x8, 0x0, 0x6, 0xaf, 0xa8, 0x0, 0xc, 0x34, 0x8, 0x10, 0x0, 0xaf, 0xa8, 0x0, 0x10, 0x3c, 0x1f, 0x0, 0x0, 0x8f, 0xff, 0x0, 0x28, 0x3, 0xe0, 0x0, 0x8, 0x0, 0x0, 0x0, 0x0})
	if err := binary.Write(loader, binary.BigEndian, st.PC); err != nil {
		return fmt.Errorf("failed to write program entry point to loader: %w", err)
	}
	st.Memory.SetMemoryRange(0, loader)
	st.PC = 0
	st.NextPC = st.PC + 4
